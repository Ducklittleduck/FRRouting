/* ip_mroute_dist => "[no] ip mroute A.B.C.D/M$prefix <A.B.C.D$gate|INTERFACE$ifname> [{(1-255)$distance|bfd$bfd [{multi-hop$bfd_multi_hop|source A.B.C.D$bfd_source|profile BFDPROF$bfd_profile}]}]" */
...
#define funcdecl_ip_mroute_dist static int ip_mroute_dist_magic(\
	...
	const char * bfd,\
	const char * bfd_multi_hop,\
	struct in_addr bfd_source,\
	const char * bfd_source_str __attribute__ ((unused)),\
...
DEFUN_CMD_FUNC_TEXT(ip_mroute_dist)
{
	...
	const char *bfd = NULL;
	const char *bfd_multi_hop = NULL;
	struct in_addr bfd_source = { INADDR_ANY };
	const char *bfd_source_str = NULL;
	const char *bfd_profile = NULL;
	...
	for (_i = 0; _i < argc; _i++) {
		...
		if (!strcmp(argv[_i]->varname, "bfd")) {
			bfd = (argv[_i]->type == WORD_TKN) ? argv[_i]->text : argv[_i]->arg;
		}
		if (!strcmp(argv[_i]->varname, "bfd_multi_hop")) {
			bfd_multi_hop = (argv[_i]->type == WORD_TKN) ? argv[_i]->text : argv[_i]->arg;
		}
		if (!strcmp(argv[_i]->varname, "bfd_source")) {
			bfd_source_str = argv[_i]->arg;
			_fail = !inet_aton(argv[_i]->arg, &bfd_source);
		}
		if (!strcmp(argv[_i]->varname, "bfd_profile")) {
			bfd_profile = (argv[_i]->type == WORD_TKN) ? argv[_i]->text : argv[_i]->arg;
		}
#if 1 /* anything that can fail? */
		if (_fail)
	...
	return ip_mroute_dist_magic(self, vty, argc, argv, no, &prefix, prefix_str, gate, gate_str, ifname, distance, distance_str, bfd, bfd_multi_hop, bfd_source, bfd_source_str, bfd_profile);
}
...
DEFUN_CMD_FUNC_TEXT(ip_route_address_interface)
{
	...
	return ip_route_address_interface_magic(self, vty, argc, argv, no, prefix, mask, mask_str, gate, gate_str, ifname, tag, tag_str, distance, distance_str, vrf, label, table, table_str, nexthop_vrf, onlink, color, color_str, bfd, bfd_multi_hop, bfd_source, bfd_source_str, bfd_profile);
}

/* ip_route_address_interface_vrf => "[no] ip route	<A.B.C.D/M$prefix|A.B.C.D$prefix A.B.C.D$mask> 	A.B.C.D$gate                                   	<INTERFACE|Null0>$ifname                       	[{                                             	  tag (1-4294967295)                           	  |(1-255)$distance                            	  |label WORD                                  	  |table (1-4294967295)                        	  |nexthop-vrf NAME                            	  |onlink$onlink                               	  |color (1-4294967295)                        	  |bfd$bfd [{multi-hop$bfd_multi_hop|source A.B.C.D$bfd_source|profile BFDPROF$bfd_profile}] 	  }]" */
...
DEFUN_CMD_FUNC_TEXT(ip_route_address_interface_vrf)
{
	...
	return ip_route_address_interface_vrf_magic(self, vty, argc, argv, no, prefix, mask, mask_str, gate, gate_str, ifname, tag, tag_str, distance, distance_str, label, table, table_str, nexthop_vrf, onlink, color, color_str, bfd, bfd_multi_hop, bfd_source, bfd_source_str, bfd_profile);
}

/* ip_route => "[no] ip route	<A.B.C.D/M$prefix|A.B.C.D$prefix A.B.C.D$mask> 	<A.B.C.D$gate|<INTERFACE|Null0>$ifname>        	[{                                             	  tag (1-4294967295)                           	  |(1-255)$distance                            	  |vrf NAME                                    	  |label WORD                                  	  |table (1-4294967295)                        	  |nexthop-vrf NAME                            	  |color (1-4294967295)                        	  |bfd$bfd [{multi-hop$bfd_multi_hop|source A.B.C.D$bfd_source|profile BFDPROF$bfd_profile}]           }]" */
...
DEFUN_CMD_FUNC_TEXT(ip_route)
{
	...
	return ip_route_magic(self, vty, argc, argv, no, prefix, mask, mask_str, gate, gate_str, ifname, tag, tag_str, distance, distance_str, vrf, label, table, table_str, nexthop_vrf, color, color_str, bfd, bfd_multi_hop, bfd_source, bfd_source_str, bfd_profile);
}

/* ip_route_vrf => "[no] ip route	<A.B.C.D/M$prefix|A.B.C.D$prefix A.B.C.D$mask> 	<A.B.C.D$gate|<INTERFACE|Null0>$ifname>        	[{                                             	  tag (1-4294967295)                           	  |(1-255)$distance                            	  |label WORD                                  	  |table (1-4294967295)                        	  |nexthop-vrf NAME                            	  |color (1-4294967295)                        	  |bfd$bfd [{multi-hop$bfd_multi_hop|source A.B.C.D$bfd_source|profile BFDPROF$bfd_profile}]           }]" */
...
DEFUN_CMD_FUNC_TEXT(ip_route_vrf)
{
	...
	return ip_route_vrf_magic(self, vty, argc, argv, no, prefix, mask, mask_str, gate, gate_str, ifname, tag, tag_str, distance, distance_str, label, table, table_str, nexthop_vrf, color, color_str, bfd, bfd_multi_hop, bfd_source, bfd_source_str, bfd_profile);
}
...
DEFUN_CMD_FUNC_TEXT(ipv6_route_address_interface)
{
	...
	for (_i = 0; _i < argc; _i++) {
		...
		if (!strcmp(argv[_i]->varname, "bfd_source")) {
			bfd_source_str = argv[_i]->arg;
			_fail = !inet_pton(AF_INET6, argv[_i]->arg, &bfd_source);
		}
		if (!strcmp(argv[_i]->varname, "bfd_profile")) {
			bfd_profile = (argv[_i]->type == WORD_TKN) ? argv[_i]->text : argv[_i]->arg;
		}
		if (!strcmp(argv[_i]->varname, "segments")) {
			segments = (argv[_i]->type == WORD_TKN) ? argv[_i]->text : argv[_i]->arg;
		}
#if 1 /* anything that can fail? */
		if (_fail)
			vty_out (vty, "%% invalid input for %s: %s\n",
	...
	return ipv6_route_address_interface_magic(self, vty, argc, argv, no, &prefix, prefix_str, &from, from_str, gate, gate_str, ifname, tag, tag_str, distance, distance_str, vrf, label, table, table_str, nexthop_vrf, onlink, color, color_str, bfd, bfd_multi_hop, bfd_source, bfd_source_str, bfd_profile, segments);
}

/* ipv6_route_address_interface_vrf => "[no] ipv6 route X:X::X:X/M$prefix [from X:X::X:X/M]           X:X::X:X$gate                                              <INTERFACE|Null0>$ifname                                   [{                                                           tag (1-4294967295)                                         |(1-255)$distance                                          |label WORD                                    	    |table (1-4294967295)                                      |nexthop-vrf NAME                              	    |onlink$onlink                                 	    |color (1-4294967295)                          	    |bfd$bfd [{multi-hop$bfd_multi_hop|source X:X::X:X$bfd_source|profile BFDPROF$bfd_profile}] 		|segments WORD 								             }]" */
...
DEFUN_CMD_FUNC_TEXT(ipv6_route_address_interface_vrf)
{
	...
	return ipv6_route_address_interface_vrf_magic(self, vty, argc, argv, no, &prefix, prefix_str, &from, from_str, gate, gate_str, ifname, tag, tag_str, distance, distance_str, label, table, table_str, nexthop_vrf, onlink, color, color_str, bfd, bfd_multi_hop, bfd_source, bfd_source_str, bfd_profile, segments);
}

/* ipv6_route => "[no] ipv6 route X:X::X:X/M$prefix [from X:X::X:X/M]           <X:X::X:X$gate|<INTERFACE|Null0>$ifname>                   [{                                                           tag (1-4294967295)                                         |(1-255)$distance                                          |vrf NAME                                                  |label WORD                                    	    |table (1-4294967295)                                      |nexthop-vrf NAME                                          |color (1-4294967295)                          	    |bfd$bfd [{multi-hop$bfd_multi_hop|source X:X::X:X$bfd_source|profile BFDPROF$bfd_profile}] 			|segments WORD 								             }]" */
...
DEFUN_CMD_FUNC_TEXT(ipv6_route)
{
	...
	return ipv6_route_magic(self, vty, argc, argv, no, &prefix, prefix_str, &from, from_str, gate, gate_str, ifname, tag, tag_str, distance, distance_str, vrf, label, table, table_str, nexthop_vrf, color, color_str, bfd, bfd_multi_hop, bfd_source, bfd_source_str, bfd_profile, segments);
}

/* ipv6_route_vrf => "[no] ipv6 route X:X::X:X/M$prefix [from X:X::X:X/M]           <X:X::X:X$gate|<INTERFACE|Null0>$ifname>                           [{                                                           tag (1-4294967295)                                         |(1-255)$distance                                          |label WORD                                    	    |table (1-4294967295)                                      |nexthop-vrf NAME                              	    |color (1-4294967295)                          	    |bfd$bfd [{multi-hop$bfd_multi_hop|source X:X::X:X$bfd_source|profile BFDPROF$bfd_profile}] 		|segments WORD 								             }]" */
...
DEFUN_CMD_FUNC_TEXT(ipv6_route_vrf)
{
	...
	return ipv6_route_vrf_magic(self, vty, argc, argv, no, &prefix, prefix_str, &from, from_str, gate, gate_str, ifname, tag, tag_str, distance, distance_str, label, table, table_str, nexthop_vrf, color, color_str, bfd, bfd_multi_hop, bfd_source, bfd_source_str, bfd_profile, segments);
}

#ifdef INCLUDE_MGMTD_CMDDEFS_ONLY
#else /* ifdef INCLUDE_MGMTD_CMDDEFS_ONLY */
/* debug_staticd => "[no] debug static [{events$events|route$route|bfd$bfd}]" */
...
DEFUN_CMD_FUNC_TEXT(debug_staticd)
{
	...
	return debug_staticd_magic(self, vty, argc, argv, no, events, route, bfd);
}
#endif

#ifdef INCLUDE_MGMTD_CMDDEFS_ONLY
#else /* ifdef INCLUDE_MGMTD_CMDDEFS_ONLY */
/* staticd_show_bfd_routes => "show bfd static route [json]$isjson" */
DEFUN_CMD_FUNC_DECL(staticd_show_bfd_routes)
#define funcdecl_staticd_show_bfd_routes static int staticd_show_bfd_routes_magic(\
	const struct cmd_element *self __attribute__ ((unused)),\
	struct vty *vty __attribute__ ((unused)),\
	int argc __attribute__ ((unused)),\
	struct cmd_token *argv[] __attribute__ ((unused)),\
	const char * isjson)
funcdecl_staticd_show_bfd_routes;
DEFUN_CMD_FUNC_TEXT(staticd_show_bfd_routes)
{
	...
	for (_i = 0; _i < argc; _i++) {
		...
		if (!strcmp(argv[_i]->varname, "isjson")) {
			isjson = (argv[_i]->type == WORD_TKN) ? argv[_i]->text : argv[_i]->arg;
		}
#if 0 /* anything that can fail? */
		if (_fail)
			vty_out (vty, "%% invalid input for %s: %s\n",
	...
	return staticd_show_bfd_routes_magic(self, vty, argc, argv, isjson);
}
#endif
